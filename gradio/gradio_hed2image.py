...
def process(
    ...
    with torch.no_grad():
        ...
        cond = {
            "c_concat": [control],
            "c_crossattn": [model.get_learned_conditioning([prompt1] * num_samples)],
            "c_control_crossattn": [
                model.get_learned_conditioning([prompt2] * num_samples)
            ],
        }
        un_cond = {
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            prompt1 = gr.Textbox(label="Prompt SD")
            prompt2 = gr.Textbox(label="Prompt ControlNet")
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                _apply_hed = gr.Checkbox(label="Apply HED", value=True)
                detect_resolution = gr.Slider(
                    label="HED Resolution", minimum=128, maximum=1024, value=512, step=1
                )
