...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, controls=None, only_mid_control=False, **kwargs):
        hs = []
        with torch.no_grad():
        ...
        # these are actually multiple now
        ...
        for i, module in enumerate(self.output_blocks):
            if only_mid_control or control is None:
                ...
                summed = controls[0].pop()
                for control in controls[1:]:
                    summed += control.pop()

                h = torch.cat([h, hs.pop() + summed], dim=1)
            h = module(h, emb, context)
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, style_stage_config=None, style_key=None, add_sd_context=False, num_controls=1, *args, **kwargs):
        ...
        self.control_models = nn.ModuleList([instantiate_from_config(control_stage_config) for n in range(num_controls)])
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        controls = batch[self.control_key]
        if bs is not None:
            controls = controls[:bs]
        controls = controls.to(self.device)
        print('pre einops', controls.shape)
        controls = einops.rearrange(controls, 'b n h w c -> n b c h w')
        print('post einops', controls.shape)
        controls = controls.to(memory_format=torch.contiguous_format).float()

        conditioning = dict(c_crossattn=[c], c_concat=[controls])
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, controls=None, only_mid_control=self.only_mid_control)
        else:
            cs = torch.cat(cond['c_concat'], 1)
            controls = []
            print('bcbs', cs.shape)

            for idx, control_model in enumerate(self.control_models):
                print('cs idx', cs[idx].shape)
                control = control_model(x=x_noisy, hint=cs[idx], timesteps=t, context=control_txt, style_embed=c_embed)
                control = [c * scale for c, scale in zip(control, self.control_scales)]
                controls.append(control)

            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, controls=controls, only_mid_control=self.only_mid_control)
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        # could also loop here
        log["control"] = c_cat[0] * 2.0 - 1.0
        ...
        if unconditional_guidance_scale > 1.0: # true, this is being used
            ...
            print('uc scale', unconditional_guidance_scale)
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        _, b, c, h, w = cond["c_concat"][0].shape
        shape = (self.channels, h // 8, w // 8)
        print('sample log shape', shape)
    ...
    def configure_optimizers(self):
        ...
        params = []
        for control_model in self.control_models:
            params += list(control_model.parameters())
    ...
    def low_vram_shift(self, is_diffusing):
        if is_diffusing:
            ...
            self.control_models = [control_model.cuda() for control_model in self.control_models]
            self.first_stage_model = self.first_stage_model.cpu()
            self.cond_stage_model = self.cond_stage_model.cpu()
        else:
            self.model = self.model.cpu()
            self.control_models = [control_model.cpu() for control_model in self.control_models]
