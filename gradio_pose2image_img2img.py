...
def process(input_image, style_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, style_strength, scale, seed, eta):
    with torch.no_grad():
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(source='upload', type="numpy", label="pose")
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                style_strength = gr.Slider(label="Style Strength", minimum=0.0, maximum=1.0, value=0.5, step=0.01)
    ...
    ips = [input_image, style_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, style_strength, scale, seed, eta]
